...
namespace SynapseStudioWeb.Controllers
{
    public partial class FormularyController : Controller
    {
        ...
        public async Task<IActionResult> SearchAMPByName(string q)
        {
            ...
            var orderedFormularies = formularies
                .Where(rec=> rec.RecStatusCode == TerminologyConstants.DRAFT_STATUS_CD || rec.RecStatusCode == TerminologyConstants.ACTIVE_STATUS_CD)
                ?.OrderByDescending(rec=> rec.Createddate).ToList();
            if (!orderedFormularies.IsCollectionValid()) return Json(null);
            formularies = new List<FormularyHeaderAPIModel>();
            orderedFormularies.Each(rec =>
            {
                if (!uniqueCodes.Contains(rec.Code))
                {
        ...
        [HttpGet]
        [Route("Formulary/GetRecordStatusLookup")]
        public async Task<JsonResult> GetRecordStatusLookup()
        {
            string token = HttpContext.Session.GetString("access_token");

            var recordStatuses = await TerminologyAPIService.GetRecordStatusLookup(token);

            return Json(recordStatuses);
        }

        [HttpGet]
        [Route("Formulary/GetFormularyStatusLookup")]
        public async Task<JsonResult> GetFormularyStatusLookup()
        {
            string token = HttpContext.Session.GetString("access_token");

            var formularyStatuses = await TerminologyAPIService.GetFormularyStatusLookup(token);

            return Json(formularyStatuses);
        }

        [HttpGet]
        [Route("Formulary/SearchTitrationType")]
        public async Task<JsonResult> SearchFDBClassifications(string q)
        {
            if (q.IsEmpty()) return Json(null);
        ...
        [HttpGet]
        [Route("Formulary/SearchFDBTherapeuticClasses")]
        public async Task<JsonResult> SearchFDBTherapeuticClasses(string q)
        {
            if (q.IsEmpty()) return Json(null);

            var fdbThereapeuticClassesInSession = await _formularyLookupService.GetFDBTherapeuticClasses();

            if (!fdbThereapeuticClassesInSession.IsCollectionValid())
            {
                return Json(null);
            }

            var matchingfdbThereapeuticClassesInSession = fdbThereapeuticClassesInSession.Where(rec => (rec.Key != null && rec.Key.ToLower().StartsWith(q.ToLower()) || (rec.Value != null && rec.Value.ToLower().StartsWith(q.ToLower()))))
                .ToList();

            if (!matchingfdbThereapeuticClassesInSession.IsCollectionValid())
                return Json(null);

            var fdbThereapeuticClassesAsLkp = matchingfdbThereapeuticClassesInSession.Select(rec => new CodeNameSelectorModel { Id = rec.Key, Name = rec.Value, IsReadonly = false, Source = TerminologyConstants.MANUAL_DATA_SOURCE, SourceColor = TerminologyConstants.GetColorForRecordSource(TerminologyConstants.MANUAL_DATA_SOURCE) });

            return Json(fdbThereapeuticClassesAsLkp);
        }

        [HttpGet]
        [Route("Formulary/SearchATCLookups")]
        public async Task<JsonResult> SearchATCLookups(string q)
        {
            if (q.IsEmpty()) return Json(null);

            var atcsInSession = await _formularyLookupService.GetATCLookup();

            if (!atcsInSession.IsCollectionValid())
            {
                return Json(null);
            }

            var matchingATCsInSession = atcsInSession.Where(rec => (rec.Key != null && rec.Key.ToLower().StartsWith(q.ToLower()) || (rec.Value != null && rec.Value.ToLower().StartsWith(q.ToLower())))).ToList();

            if (!matchingATCsInSession.IsCollectionValid())
                return Json(null);

            var atcsAsLkp = matchingATCsInSession.Select(rec => new CodeNameSelectorModel { Id = rec.Key, Name = rec.Value, IsReadonly = false, Source = TerminologyConstants.MANUAL_DATA_SOURCE, SourceColor = TerminologyConstants.GetColorForRecordSource(TerminologyConstants.MANUAL_DATA_SOURCE) });

            return Json(atcsAsLkp);
        }

        [HttpGet]
        [Route("Formulary/SearchBNFLookups")]
        public async Task<JsonResult> SearchBNFLookups(string q)
        {
            if (q.IsEmpty()) return Json(null);

            var bnfsInSession = await _formularyLookupService.GetBNFLookup();

            if (!bnfsInSession.IsCollectionValid())
            {
                return Json(null);
            }

            var matchingBNFsInSession = bnfsInSession.Where(rec => (rec.Key != null && rec.Key.ToLower().StartsWith(q.ToLower()) || (rec.Value != null && rec.Value.ToLower().StartsWith(q.ToLower()))))
                .ToList();

            if (!matchingBNFsInSession.IsCollectionValid())
                return Json(null);

            var bnfsAsLkp = matchingBNFsInSession.Select(rec => new CodeNameSelectorModel { Id = rec.Key, Name = rec.Value, IsReadonly = false, Source = TerminologyConstants.MANUAL_DATA_SOURCE, SourceColor = TerminologyConstants.GetColorForRecordSource(TerminologyConstants.MANUAL_DATA_SOURCE) });

            return Json(bnfsAsLkp);
        }
    }
}