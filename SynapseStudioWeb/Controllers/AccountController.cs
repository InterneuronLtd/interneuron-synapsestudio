    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Index(string access_token, string code)
        {
            ...
            if (((JwtSecurityToken)jwttoken).Claims.Count() > 1)
            {
                ...
                if(!((JwtSecurityToken)jwttoken).Claims.Any(x => string.Compare(x.Type, "synapseroles", true) == 0 && string.Compare(x.Value, "studioadmin", true) == 0) 
                    && !((JwtSecurityToken)jwttoken).Claims.Any(x => string.Compare(x.Type, "synapseroles", true) == 0 && string.Compare(x.Value, "pharmacist", true) == 0)
                    && !((JwtSecurityToken)jwttoken).Claims.Any(x => string.Compare(x.Type, "synapseroles", true) == 0 && string.Compare(x.Value, "epma super user", true) == 0))
                {
                    return RedirectToAction("Logout");
                }

                HttpContext.Session.SetString(SynapseSession.IsStudioAdmin,
                    ((JwtSecurityToken)jwttoken).Claims.Any(x => string.Compare(x.Type, "synapseroles", true) == 0
                    && string.Compare(x.Value, "studioadmin", true) == 0).ToString());

                HttpContext.Session.SetString(SynapseSession.IsPharamacist,
                    ((JwtSecurityToken)jwttoken).Claims.Any(x => string.Compare(x.Type, "synapseroles", true) == 0
                    && string.Compare(x.Value, "pharmacist", true) == 0).ToString());

                if(((JwtSecurityToken)jwttoken).Claims.Any(x => string.Compare(x.Type, "synapseroles", true) == 0 && string.Compare(x.Value, "epma super user", true) == 0))
                {
                    HttpContext.Session.SetString(SynapseSession.IsPharamacist,
                    ((JwtSecurityToken)jwttoken).Claims.Any(x => string.Compare(x.Type, "synapseroles", true) == 0
                    && string.Compare(x.Value, "epma super user", true) == 0).ToString());
                }
