        ...
        private void ValidateClassificationCodes()
        {
            if (_model.FormularyClassificationCodes.IsCollectionValid())
            {
                ...
                _model.FormularyClassificationCodes.Each(rec =>
                {
                    ...
                    //{
                    //    //Allowed only for FDB and bnf now
                    //    hasDups = (string.Compare(rec.AdditionalCodeSystem, "fdb", true) == 0 || string.Compare(rec.AdditionalCodeSystem, "bnf", true) == 0) ? false : true;
