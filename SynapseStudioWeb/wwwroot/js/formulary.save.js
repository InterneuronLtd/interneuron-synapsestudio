...
function processAdditionalCodes(formInput, rootElSel) {
    let newPanelId = `${rootElSel} #classification_codes`;
    if (newPanelId) {
        let dataListAsStr = $(newPanelId).prop('existingadditionalcode');
        if (!dataListAsStr) return;

        let dataList = JSON.parse(dataListAsStr);

        if (!dataList || dataList.length === 0) return;

        dataList.forEach(function (elItem, index) {
            var FormularyClassificationCodes = {};

            FormularyClassificationCodes["AdditionalCode"] = elItem.AdditionalCode;
            FormularyClassificationCodes["AdditionalCodeSystem"] = elItem.AdditionalCodeSystem;
            FormularyClassificationCodes["AdditionalCodeDesc"] = elItem.AdditionalCodeDesc;
            FormularyClassificationCodes["Source"] = elItem.Source;
            FormularyClassificationCodes["CodeType"] = elItem.CodeType;
            FormularyClassificationCodes["IsDefault"] = elItem.IsDefault;
    ...
    $(panelId).each(function (index, elItem) {
        ...
        if (additionalCodeCntrl && additionalCodeCntrl.length > 0) {
            ...
            let additionalCodeTypeIsDefault = $($(`${rootElSel} .additionalCode .additonal-code-default`)[index]).is(':checked');
            ...
            FormularyClassificationCodes["IsDefault"] = additionalCodeTypeIsDefault;
...
function processIngredients(formInput, rootElSel) {
    ...
    $(`${rootElSel} .ingredient`).each(function (index, elItem) {
        ...
        let basisOfPharmaceuticalStrengthDesc = null;
        ...
        if ($(`${rootElSel} .ingredient select`) && $(`${rootElSel} .ingredient select`).length && $(`${rootElSel} .ingredient select`)[index].selectedOptions.length > 0) {
            ...
            let basisOfPharmaceuticalStrengthDescCntrl = $(`${rootElSel} .ingredient input[id^='${rootElName}_BasisOfPharmaceuticalStrengthDesc']`);
            if (basisOfPharmaceuticalStrengthDescCntrl && basisOfPharmaceuticalStrengthDescCntrl[index]) {
                basisOfPharmaceuticalStrengthDesc = $(basisOfPharmaceuticalStrengthDescCntrl[index]).val();
            }
        }
        ...
        Ingredients["BasisOfPharmaceuticalStrengthDesc"] = basisOfPharmaceuticalStrengthDesc;
...
studio.FormularySave = studio.FormularySave || class {
    ...
    update(beforeSaveFn, saveSuccessFn, saveFailFn) {
        ...
        let updateConfirmationurl = isBulkEdit ? 'Formulary/NeedUserConfirmationForBulkUpdate' : 'Formulary/NeedUserConfirmationForUpdate';

        ajaxPost(updateConfirmationurl, formInput,
            (data) => {
                if (data && data.needConfirmation) {
                    $.confirm(data.msg, (cmd) => {
                        if (cmd === 'yes') {
                            ...
                            if (saveFailFn) saveFailFn('', formInput);
                        }
                    });
                } else if (!data || !data.needConfirmation) {
                    ajaxPost(url, formInput,
                        (data) => {
                            if (saveSuccessFn) saveSuccessFn(data, formInput);
                        },
                ...
                console.error('Error In the API Formulary/NeedUserConfirmationForUpdate invocation');
                console.error(err);
            });
