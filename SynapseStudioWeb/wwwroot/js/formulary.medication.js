            ...
            renderNode: function (event, data) {
                ...
                if (nodeVal && nodeVal.data) {
                    ...
                    if (nodeLevel == 'AMP' && recStatusCode === '001') {
                        //nodeVal.unselectable = true;//MMC-477 - should allow since automatically move the eixisting draft to archive
...
function checkBackgroundaskStatus(taskId, onStatusCallback) {
    ajaxGetJson(`/Formulary/GetTaskByTaskId`, {taskId},
        (data) => {
            onStatusCallback({ data, err: null });
    },
        (err) => {
            onStatusCallback({ data: null, err });
            toastr.error(`Error getting the task status for TaskId: {taskId}.`);
            console.error(err);
        });
}
var onPostImportSuccess = null;
...
function importDMDs() {
    ...
    for (var i = 0; i < selectedItems.length; i++) {
        ...
        //MMC-477 - consider only AMP
        if (selectedItem && selectedItem.Level === 'AMP')
            meds.push(selectedItem.Key);

        /*MMC-477 - Not required for any parent but only AMPs
    ...
    if (!(meds && meds.length)) {
        toastr.error(`No records considered for import.`);
        return;
    }
        ...
        (data) => {
            if (!data) {//Due to some error
                ...
                var bgInterval = setInterval(() => {
                    checkBackgroundaskStatus(data, (res, err) => {
                        console.log('data returned for bg task', res, err);
                        console.log('data returned for bg task in res=', res.data.statusCd, res.data);
                        if (err || !res || !res.data) {
                            clearInterval(bgInterval);
                            $('#pnlUpdateProgress').hide();
                            $('#medModal').modal('hide');
                            toastr.error(`Error getting the task status for TaskId: UNKNOWN.`);
                        } else if (res && res.data && (res.data.statusCd === 3 || res.data.statusCd === 4)) {
                            clearInterval(bgInterval);
                            console.log('data returned for bg task again', res, err);

                            if (res.data.statusCd === 3) {
                                toastr.success(`Successfully completed import process for TaskId: ${res.data.taskId}.`);
                                //MMC-477
                                if (onPostImportSuccess) onPostImportSuccess();
                                //if (callSearchFormularies) callSearchFormularies();
                            }
                            if (res.data.statusCd === 4)
                                toastr.error(`Error getting the task status for TaskId: ${res.data.taskId}.`);
        ...
        (data) => {
            ...
            for (let resIndex = 0; resIndex < data.length; resIndex++) {
                ...
                if (node) {
                    ...
                    //MMC-477 - added condition
                    if (node.data && node.data['Level'] === 'AMP' && dataItem.recStatusCode && (dataItem.recStatusCode === '001' || dataItem.recStatusCode === '003')) 
                        node.addClass("dmdMedBackground");
                    if (node.data)
