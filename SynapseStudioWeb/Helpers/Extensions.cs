...
namespace SynapseStudioWeb.Helpers
{
    public static class Extensions
    {
        ...
        public static string SerializeSelectListToJsonArray(this SelectList selectList)
        {
            var list = new List<FormularyAdditionalCodeModel>();

            if (selectList == null || selectList.Count() == 0) return String.Empty;
        ...
        public static Nullable<T> ToNullable<T>(this string s) where T : struct
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(s) && s.Trim().Length > 0)
                {
                    ...
                    var convTypeName = conv.GetType() != null ? conv.GetType().Name : "";
                    if (((string.Compare(s,"0", true) == 0) || (string.Compare(s, "1", true) == 0)) && (string.Compare(convTypeName, "BooleanConverter", true) == 0))
                        s = (string.Compare(s, "1", true) == 0) ? "true" : "false";
        ...
        public static JArray? TryConvertToJArray(this string content)
        {
            try
            {
